...
public class SettingController : Controller
{
    ...
    public IActionResult Picture(IFormFile filename)
    {
        if (filename != null && filename.Length > 0)
        {
            ...
            var userToUpdate = users.FirstOrDefault(user => user.Name == HttpContext.Session.GetString("Name"));
            ...
            foreach (var act in acts) {
                ...
                if (user != null) {
                    user.Users.Pic = relativePath;
                }
            }
            var updatedJson2 = JsonSerializer.Serialize(acts);

            System.IO.File.WriteAllText("./Database/User.json", updatedJson);
            System.IO.File.WriteAllText("./Database/Activity.json", updatedJson2);
